...
func (l *CronAuditLog) EndLog() {
	...
	fmt.Printf(
		"token: %s | name: %s | timestamp: %s | execution Time: %d (internal: %d, external: %d) | cron job ending\n",
		l.RequestToken, l.Name, l.End.Format(common.DateTimeFormat), l.ExecTime, l.InternalExecTime, l.ExternalExecTime,
	)
}

func (l *CronAuditLog) GetLogs() *[]execLog {
	return l.ExecutionLogs
}

func (l *CronAuditLog) SetResponse(rc string, rd string, data map[string]any) {
	l.ResponseCode = rc
